...
export const AccountSettings = ({ setActiveTab }) => {
	...
	useEffect(() => {
		const fetchProfile = async () => {
			try {
				const response = await api.get(PARENT_EMAIL_GET);
	...
	const handlePasswordChange = async () => {
		...
		try {
			const response = await api.post(PARENT_PASS_CHANGE, {
				currentPassword,
				newPassword,
			});
	...
	const handleSaveClick = async () => {
		try {
			const response = await api.post(PARENT_EMAIL_CHANGE, { newEmail: email });
	...
	return (
		<div className="bg-white w-full h-full rounded-xl overflow-y-auto">
			...
			<div className="mx-20 space-y-4">
				...
				{errorMessage && (
					<div className="p-4 mb-4 text-red-800 bg-red-100 rounded">{errorMessage}</div>
				)}
